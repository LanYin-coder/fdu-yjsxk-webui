...
try:
    from . import grab  # 作为 src 包被 WebUI 导入
except ImportError:
    import grab  # 直接运行 python src/preselect.py
...
HOME_URL = f"http://{grab.DOMAIN}{grab.APP_ENTRY}"
STUDENT_INFO_URL = (
    f"http://{grab.DOMAIN}/yjsxkapp/sys/xsxkappfudan/"
    "xsxkHome/loadStdInfoSimple.do"
)
COURSE_SCOPE_URL = (
    f"http://{grab.DOMAIN}/yjsxkapp/sys/xsxkappfudan/"
    "xsxkHome/loadAllJxbKxfw.do"
)
...
def _matches(rule_value, student_value) -> bool:
    """Mirror courses.js l(): blank permits all, otherwise match one CSV value."""
    if _blank(rule_value):
        return True
    return str(student_value) in str(rule_value).split(",")


def _overlaps(rule_value, student_value) -> bool:
    """Mirror courses.js d() for the multi-value student-tag field."""
    if _blank(rule_value):
        return True
    if _blank(student_value):
        return False
    return bool(set(str(rule_value).split(",")) & set(str(student_value).split(",")))
...
def course_in_student_scope(
    rules: list[dict] | None,
    student: dict,
    server_time: int,
    allow_empty: bool,
) -> bool:
    """Apply FDU courses.js filterDatasByXkfw to one teaching class."""
    if not rules:
        return allow_empty

    active_rules = []
    for rule in rules:
        start = _as_int(rule.get("KSSJTIMES"))
        end = _as_int(rule.get("JSSJTIMES"))
        if start == 0 or end == 0 or start <= server_time <= end:
            active_rules.append(rule)
    if not active_rules:
        return True

    for rule in active_rules:
        if not all(_matches(rule.get(rule_key), student.get(student_key))
                   for rule_key, student_key in ALLOW_FIELDS):
            continue
        if not _overlaps(rule.get("KXXSBQ"), student.get("XSBQ")):
            continue

        graduation = _as_int(student.get("YJBYSJ_INT"), -1)
        lower = rule.get("YJBYSJ1_INT")
        upper = rule.get("YJBYSJ2_INT")
        if not _blank(lower) and graduation < _as_int(lower):
            continue
        if not _blank(upper) and graduation > _as_int(upper):
            continue

        specified_exclusions = [
            _matches(rule.get(rule_key), student.get(student_key))
            for rule_key, student_key in EXCLUDE_FIELDS
            if not _blank(rule.get(rule_key))
        ]
        if specified_exclusions and all(specified_exclusions):
            continue
        return True
    return False


def fetch_eligibility_context(cookie: str, timeout: float = 12.0) -> tuple[dict, dict, int, bool]:
    """Fetch the same student and class-scope data used by FDU's course page."""
    headers = {
        "Cookie": cookie,
        "User-Agent": grab.UA,
        "Referer": HOME_URL,
    ...
    try:
        home = session.get(HOME_URL, headers=headers, timeout=timeout, allow_redirects=False)
        student_response = session.get(
            f"{STUDENT_INFO_URL}?_={int(time.time() * 1000)}",
            headers=headers,
            timeout=timeout,
        )
        scope_response = session.get(
            f"{COURSE_SCOPE_URL}?_={int(time.time() * 1000)}",
            headers=headers,
            timeout=timeout,
        )
    except requests.RequestException as exc:
        raise SystemExit(f"✗ 请求当前学生可选范围失败：{exc}")

    if home.status_code in (301, 302, 303, 307, 308):
        raise SystemExit("✗ 登录态已失效，无法读取当前学生可选范围")
    if home.status_code != 200 or student_response.status_code != 200 or scope_response.status_code != 200:
        raise SystemExit("✗ 当前学生可选范围接口返回异常")
    try:
        student = student_response.json().get("xs")
        scopes = scope_response.json().get("xsfwList")
    except Exception:
        raise SystemExit("✗ 当前学生可选范围响应不是 JSON，可能登录态失效")
    if not isinstance(student, dict) or not isinstance(scopes, dict):
        raise SystemExit("✗ 当前学生可选范围响应结构异常，已停止展示课程")

    time_match = re.search(r"WIS_DQSJ\s*=\s*parseInt\(['\"](\d{14})", home.text)
    empty_match = re.search(r"WIS_JXBXKXZWKSFYXXZ\s*=\s*['\"]([01])", home.text)
    server_time = int(time_match.group(1)) if time_match else int(dt.datetime.now().strftime("%Y%m%d%H%M%S"))
    allow_empty = bool(empty_match and empty_match.group(1) == "1")
    return student, scopes, server_time, allow_empty
...
def fetch_lists(cookie: str, timeout: float = 12.0) -> list[tuple[str, int, str, int, list[dict]]]:
    ...
    student, scopes, server_time, allow_empty = fetch_eligibility_context(cookie, timeout)
    excluded_total = 0
    last_group = None
    for group, lx, label, endpoint, tab, val in LISTS:
        ...
        rows = [
            row for row in rows
            if course_in_student_scope(scopes.get(str(row.get("BJDM") or "")), student, server_time, allow_empty)
        ]
        excluded_total += raw_count - len(rows)
        out.append((group, lx, label, val, rows))
        suffix = f"（排除不可选 {raw_count - len(rows)} 门）" if raw_count != len(rows) else ""
        log(f"  · {label}：{len(rows)} 门{suffix}")
    if excluded_total:
        log(f"  · 已按当前学生资格范围合计排除 {excluded_total} 个不可选班次")
    return out
...
def main() -> int:
    ...
    if not courses:
        ...
        return 0
    if len(courses) > grab.MAX_BURST_COURSES:
        log(f"✗ 批量提交最多支持 {grab.MAX_BURST_COURSES} 门课程，当前选择了 {len(courses)} 门")
        log("请重新运行预选课并减少课程数量；本次未写入 config.json")
        return 1
